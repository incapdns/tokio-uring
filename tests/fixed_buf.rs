...
fn fixed_buf_turnaround() {
  tokio_uring::start(async {
    ...
    let mut buffers = FixedBufRegistry::new([30, 20, 10].iter().map(|&n| Vec::with_capacity(n)));
...
fn unregister_invalidates_checked_out_buffers() {
  tokio_uring::start(async {
    ...
    let mut buffers = FixedBufRegistry::new([Vec::with_capacity(1024)]);
    buffers.register().unwrap();
...
fn pool_next_as_concurrency_limit() {
  tokio_uring::start(async move {
    ...
    let mut buffers = FixedBufPool::new(iter::repeat_with(|| Vec::with_capacity(BUF_SIZE)).take(2));
    buffers.register().unwrap();
